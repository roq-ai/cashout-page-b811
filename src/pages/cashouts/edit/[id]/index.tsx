...
import { getCashoutById, updateCashoutById } from 'apiSdk/cashouts';
...
function CashoutEditPage() {
  ...
  const handleSubmit = async (values: CashoutInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCashoutById(id, values);
      mutate(updated);
      resetForm();
