...
import { getCashoutById } from 'apiSdk/cashouts';
import { CashoutInterface } from 'interfaces/cashout';

function CashoutViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<CashoutInterface>(
    () => (id ? `/cashouts/${id}` : null),
    () =>
      getCashoutById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('cashout', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/cashouts/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Cashout Date"
                  text={
                    data?.cashout_date ? format(parseISO(data?.cashout_date as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
